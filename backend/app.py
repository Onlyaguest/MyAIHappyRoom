...
def load_team_status():
    defaults = _default_team_roles()
    by_default_id = {item["roleId"]: item for item in defaults}

    payload = None
    if os.path.exists(TEAM_STATUS_FILE):
        try:
    ...
    if isinstance(rows, list):
        for row in rows:
            ...
            if role_id not in by_default_id:
                continue
            base = dict(by_default_id[role_id])
            state = normalize_agent_state(row.get("state"))
            updated_at = str(row.get("updated_at") or "").strip() or datetime.now().isoformat()
            base.update({
                "name": str(row.get("name") or base["name"]).strip() or base["name"],
                "role": str(row.get("role") or base["role"]).strip() or base["role"],
                "state": state,
                "note": str(row.get("note") or "").strip(),
                "updated_at": updated_at,
            })
            normalized[role_id] = base

    merged = []
    for d in defaults:
        merged.append(normalized.get(d["roleId"], d))
    return merged


def save_team_status(roles):
    safe_roles = []
    for row in (roles or []):
        if not isinstance(row, dict):
            continue
        ...
        safe_roles.append({
            "roleId": role_id,
            "name": str(row.get("name") or "").strip() or role_id.title(),
    ...
    by_id = {r["roleId"]: r for r in safe_roles}
    final_rows = []
    for d in _default_team_roles():
        final_rows.append(by_id.get(d["roleId"], d))
...
@app.route("/team-status", methods=["POST"])
def set_team_status():
    try:
        data = request.get_json()
        if not isinstance(data, dict):
        ...
        roles = load_team_status()
        by_id = {r.get("roleId"): r for r in roles}
        ...
        if isinstance(data.get("roles"), list):
            ...
            for rid in ("pm", "builder", "reviewer"):
                ...
                current["roleId"] = rid
                current["name"] = str(patch.get("name") or current.get("name") or rid.title()).strip() or rid.title()
                current["role"] = str(patch.get("role") or current.get("role") or "").strip()
                current["state"] = normalize_agent_state(patch.get("state") or current.get("state"))
                if "note" in patch:
                    current["note"] = str(patch.get("note") or "").strip()
                else:
                    current["note"] = str(current.get("note") or "").strip()
                current["updated_at"] = datetime.now().isoformat()
                merged.append(current)
            save_team_status(merged)
            return jsonify({"ok": True, "roles": load_team_status()})
        ...
        target["role"] = str(data.get("role") or target.get("role") or "").strip()
        target["state"] = normalize_agent_state(data.get("state") or target.get("state"))
        if "note" in data:
            target["note"] = str(data.get("note") or "").strip()
        else:
            target["note"] = str(target.get("note") or "").strip()
...
@app.route("/one-click-summary", methods=["POST"])
def one_click_summary():
    """Generate role-status summary.

    First version keeps rule-based summarization and can be replaced by LLM later.
    """
    try:
        ...
        if not isinstance(incoming, list) or len(incoming) == 0:
            ...
            for row in incoming:
                if not isinstance(row, dict):
                    continue
                roles.append({
                    "roleId": str(row.get("roleId") or "").strip().lower(),
