...
## MyAIHappyRoom 改造说明（Phase 1 / 2）

- 本仓库 `https://github.com/Onlyaguest/MyAIHappyRoom` 以 `https://github.com/ringhyacinth/Star-Office-UI` 为基底导入。
- 后续所有新改动只在 `MyAIHappyRoom` 提交，不回写 `Star-Office-UI`。
- 当前已增加：
...
```bash
git clone https://github.com/Onlyaguest/MyAIHappyRoom.git
cd MyAIHappyRoom
...
- 外部状态桥接（P0）：`docs/external-status-bridge.md`
